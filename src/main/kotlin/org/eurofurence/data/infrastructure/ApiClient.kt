...
import okhttp3.*
import org.eurofurence.data.auth.Authentication
import java.io.File
...
open class ApiClient(val baseUrl: String) {
    companion object {
        ...
        protected val OctetStreamMediaType = "application/octet-stream"
        protected val TextMediaType = "text/*"
    ...
    val authentication: ThreadLocal<Authentication?> = ThreadLocal.withInitial { null }
    ...
    inline protected fun <reified T : Any?> responseBody(body: ResponseBody?, mediaType: String = JsonMediaType): T? {
        ...
        return when (mediaType) {
            ...
            TextMediaType -> when (T::class) {
                String::class -> body.string() as T
                else -> error("Incompatible class ${T::class}")
            }
            OctetStreamMediaType -> when (T::class) {
    ...
    inline protected fun <reified T : Any?> prepareRequest(requestConfig: RequestConfig): RequestConfig {
        // Change the accept type based on the given T
        val changeAccept = when (T::class) {
            ByteArray::class -> requestConfig.addHeader(Accept, OctetStreamMediaType)
            String::class -> requestConfig.addHeader(Accept, TextMediaType)
            else -> requestConfig
        }

        // Change the authentication if set
        val changeAuth = authentication.get()?.apply(changeAccept) ?: changeAccept
    ...
    inline protected fun <reified T : Any?> request(requestConfig: RequestConfig, body: Any? = null): ApiInfrastructureResponse<T?> {
        // Patch request
        val actualConfig = prepareRequest<T>(requestConfig)

        val httpUrl = HttpUrl.parse(baseUrl) ?: throw IllegalStateException("baseUrl is invalid.")

        var urlBuilder = httpUrl.newBuilder()
                .addPathSegments(actualConfig.path.trimStart('/'))

        actualConfig.query.forEach { query ->
        ...
        val headers = defaultHeaders + actualConfig.headers
        ...
        var request: Request.Builder = when (actualConfig.method) {
